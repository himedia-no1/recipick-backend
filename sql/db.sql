...
CREATE OR REPLACE FUNCTION optimized_korean_similarity(ingredient TEXT, category TEXT) RETURNS FLOAT AS
$$
BEGIN
    IF ingredient = category THEN RETURN 1.0; END IF;
    IF category LIKE ingredient || '%' THEN RETURN 0.95; END IF;
    IF category LIKE '%' || ingredient THEN RETURN 0.9; END IF;
    IF category LIKE '%' || ingredient || '%' THEN RETURN 0.85; END IF;
    IF ingredient LIKE '%' || category || '%' THEN RETURN 0.8; END IF;

    IF ABS(LENGTH(ingredient) - LENGTH(category)) > 3 THEN RETURN 0.0; END IF;

    IF levenshtein(ingredient, category) <= 2 THEN RETURN 0.75 - (levenshtein(ingredient, category) * 0.1); END IF;
...
CREATE TABLE recipes (
    id              INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    search_identity VARCHAR(10) NOT NULL,
...
CREATE UNIQUE INDEX uniq_recipes_id_active ON recipes (id) WHERE is_deleted IS FALSE;;
...
CREATE TABLE recipe_steps (
    ...
    image       VARCHAR(500),
    CONSTRAINT fk_recipe_steps_recipe_id FOREIGN KEY (recipe_id) REFERENCES recipes (id)
);
CREATE INDEX idx_recipe_steps_recipe_id ON recipe_steps (recipe_id);
COMMENT ON TABLE recipe_steps IS '레시피 조리단계 테이블';
...
CREATE UNIQUE INDEX uniq_admins_email_active ON admins (email) WHERE is_deleted IS FALSE;
CREATE TRIGGER trg_admins_timestamps_at
    BEFORE INSERT OR UPDATE
    ON admins
    FOR EACH ROW
EXECUTE FUNCTION trg_set_timestamps_at();
COMMENT ON TABLE ingredients IS '관리자 테이블';